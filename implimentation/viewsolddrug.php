...
<li><a href="#">View<font color="red"></a>
 <ul>
		...
		<li><a href="viewcustinfo.php">@@&nbsp;Customer History</a></li>
		  <li><a href="viedrug.php">@@&nbsp;View stock Drug</font></a></li>
...
$que="SELECT * FROM solddrug where date='{$ddate}' AND status = 'true' AND audited='No' ";
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<tr>";
  $bach=$row['bachnumber'];
  $batch="select * from stockdrug where bachnumber='{$bach}'";
   $bach=mysqli_query($conn,$batch);
  ...
  echo "<td>" . $row['quantity'] . "</td>";
   echo "<td>" . $col['unitprice'] . "</td>";
	...
	$sum = $sum + ($row['quantity'] * $col['unitprice']);
	 ...
	 $up1="UPDATE solddrug SET audited='Yes' WHERE status = 'true' ";
         $up=mysqli_query($conn,$up1);
...
echo"</font>&nbsp;&nbsp;&nbsp;&nbsp;<input type='text' value= $sum readonly size=5>";
}
